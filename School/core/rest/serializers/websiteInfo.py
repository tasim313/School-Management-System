...
class SchoolWebsiteCreateSerializer(serializers.Serializer):
    ...
    divisions = serializers.CharField(
        max_length=255,
    ...
    pourashava = serializers.CharField(
        max_length=255,
        trim_whitespace=True,
        required=False,
        label="Pourashava",
    ...
    union_parishad = serializers.CharField(
        max_length=255,
        trim_whitespace=True,
        required=False,
    ...
    house_holding_number = serializers.CharField(
        max_length=255,
        trim_whitespace=True,
        required=False,
    ...
    phone = PhoneNumberField(
        required=False,
        allow_blank=True,
        label="School Website Phone Number or Contact Number",
    )
...
class SchoolWebsiteUpdateSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=Status.choices, required=False, label="Status"
    )
    ...
    def update(self, instance, validated_data):
        ...
        school_address.district = validated_data.get(
            "district", school_address.district
        )
        school_address.upazila = validated_data.get("upazila", school_address.upazila)
        school_address.pourashava = validated_data.get(
            "pourashava", school_address.pourashava
        )
        school_address.union_parishad = validated_data.get(
            "union_parishad", school_address.union_parishad
        )
        ...
        school_address.house_holding_number = validated_data.get(
            "house_holding_number", school_address.house_holding_number
        )
        school_address.post_office = validated_data.get(
            "post_office", school_address.post_office
        )
        school_address.post_code = validated_data.get(
            "post_code", school_address.post_code
        )
...
class SchoolWebsiteGallerySerializer(serializers.Serializer):
    uid = serializers.UUIDField(format="hex_verbose", write_only=True)
    image = serializers.ImageField(
        max_length=None,
        allow_empty_file=False,
        use_url=get_website_get_gallery_image,
        label="Image",
        required=False,
    )
