...
def get_website_home_slider_content_image(instance, filename):
    uid = str(uuid4()).split("-")[-1]
    return f"{uid}{'website-home-slider'}{'slider'}-{filename}{str(instance.uid).split('-')[0]}"

def get_website_about_file_image(instance, filename):
    uid = str(uuid4()).split("-")[-1]
    return f"{uid}{'website-about-image'}{'image'}-{filename}{str(instance.uid).split('-')[0]}"

def get_website_about_file_awards_image(instance, filename):
    uid = str(uuid4()).split("-")[-1]
    return f"{uid}{'website-awards-image'}{'image'}-{filename}{str(instance.uid).split('-')[0]}"

def get_website_about_file_awards_image(instance, filename):
    uid = str(uuid4()).split("-")[-1]
    return f"{uid}{'website-awards-image'}{'awards'}-{filename}{str(instance.uid).split('-')[0]}"
